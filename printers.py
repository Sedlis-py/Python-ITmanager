...
class Printers_frame(ctk.CTkFrame):
    ...
    def create_pdf(self):
        ...
        for i, column in enumerate(columns):  # Start od druhého sloupce
    ...
    def hardware_table_show(self):
        ...
        # BUTTON
        button_hardware_add = ctk.CTkButton(frame_buttons, text="Vložit nové zařízení", width=10, command=self.hardware_add_event)
        button_hardware_add.configure(font=("Arial", 12, "bold"))
        button_hardware_add.grid(row=0, column=0, pady=(0, 0), padx=(0, 0))

        button_hardware_find = ctk.CTkButton(frame_buttons, text="Vyhledat zařízení", width=10, command=self.find_hardware)
        button_hardware_find.configure(font=("Arial", 12, "bold"))
        button_hardware_find.grid(row=0, column=1, pady=(0, 0), padx=(10, 0), sticky="w")

        button_hardware_print = ctk.CTkButton(frame_buttons, text="Tisk seznam zařízení", width=10, command=self.create_pdf)
        ...
        label_description = ctk.CTkLabel(frame_buttons, text="Přehled tiskáren")
        ...
        self.tree = ttk.Treeview(self, columns=("hardware_ip", "hardware_type", "hardware_mark", "hardware_place", "hardware_name", "hardware_link", "hardware_info"), show='headings', height=25)
    ...
    def lock_columns(self):
        # Uzamčení sloupců, aby nebylo možno měnit jejich šířku
    ...
    def on_detail_window_close(self, window):
        self.reset_row_colors() # Resetování barvy řádku, když je detail okna uzavřen
        window.destroy()
    ...
    def hardware_add_event(self):
        ...
        globals.hardware_event.title("Informace o nové tiskárně")
        ...
        def hardware_add():
            ...
            if ip != "-":
                try:
                    ipaddress.ip_address(ip)  # Pokus o vytvoření instance ip_adresy, aby se otestovalo, jestli se jedná o IP adresu
                except ValueError:
                ...
                if existing_ip:
                    ...
                    if (
                            textbox_hardware_ip.get().strip() != "" and textbox_hardware_mark.get().strip() != "" and textbox_hardware_place.get().strip() != "" and
                        ...
                        hardware_message_error("Chyba!",
                                               "Nejsou vyplněna správně všechna pole. \nProsím zkontrolujte formulář.")
        ...
        # Main header
        label_header = ctk.CTkLabel(globals.hardware_event, text="Informace o nové tiskárně")
        ...
        label_hardware_ip = ctk.CTkLabel(frame_hardware_ip, text="IP adresa")
        ...
        textbox_hardware_ip.grid(row=0, column=1, pady=(0,0), padx=(105,0), sticky="w")
        ...
        combobox_hardware_type = ctk.CTkComboBox(frame_hardware_type, values=["-    typ zařízení  -", "Tiskárna"], width=200, justify="center")
    ...
    def hardware_edit_event(self, index):
        ...
        globals.hardware_event = ctk.CTkToplevel(fg_color="#242222")
        globals.hardware_event.title("Změna údajů tiskárny")
        globals.hardware_event.resizable(width=False, height=False)
        globals.hardware_event_open = True
            ...
            if ip != "-":
                try:
                    ipaddress.ip_address(
                        ip)  # Pokus o vytvoření instance ip_adresy, aby se otestovalo, jestli se jedná o IP adresu
                except ValueError:
                    hardware_message_error("Špatná IP adresa", f"Zadaná IP adresa '{ip}' není platná.")
                    return
                ...
                if check_hardware is not None and check_hardware[0] != id:
                    hardware_message_error("Upravit záznam",
                                           "Bohužel nově zadaná adresa z formuláře již v seznamu existuje.")
                else:
                    if (
                            textbox_hardware_ip.get().strip() != "" and textbox_hardware_mark.get().strip() != "" and textbox_hardware_place.get().strip() != "" and
                            textbox_hardware_name.get().strip() != "" and textbox_hardware_link.get().strip() != "" and textbox_hardware_info.get().strip() and
                ...
                if (
                    ...
                    cursor.execute("""
                                        UPDATE ITmanager_hardware
                                        SET hardware_ip = ?, hardware_type = ?, hardware_mark = ?, hardware_place = ?, hardware_name = ?, hardware_link = ?, 
                                        hardware_info = ? WHERE hardware_id = ?""", (ip, type, mark, place, name, link, info, id))
                    connection.commit()
                    cursor.close()
                    connection.close()
                    globals.hardware_event.destroy()
        ...
        # Main header
        label_header = ctk.CTkLabel(globals.hardware_event, text="Změna údajů tiskárny")
        label_header.configure(font=("Arial", 14, "bold"))
        label_header.pack(pady=(25, 0), padx=(0, 0))
